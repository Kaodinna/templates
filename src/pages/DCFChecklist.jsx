...
function CheckItem({ item, checked, onToggle }) {
  return (
    <div
      ...
      className={`flex items-start gap-3.5 px-3.5 py-3 -mx-3.5 rounded-lg cursor-pointer transition-all select-none
        hover:bg-cream-warm group
        ${checked ? 'opacity-70' : ''}`}>
      {/* Box */}
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-[14px] font-medium leading-snug transition-all ${checked ? 'line-through decoration-gold decoration-[1.5px] text-gray-400' : 'text-gray-700'}`}>
          {item.label}
        </p>
        {item.note && (
          <p className={`text-[12.5px] italic mt-0.5 leading-snug ${checked ? 'text-gray-300' : 'text-gray-400'}`}>
            {item.note}
          </p>
        )}
      </div>
...
function SectionCard({ section, state, onToggle, onNote }) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl px-7 py-6 mb-5 shadow-sm">
      {/* Section header */}
      <div className="pb-4 border-b border-gray-100 mb-5">
        <div className="flex items-center gap-3 mb-1.5">
          <span className="font-serif text-[13px] text-gold bg-gold-soft px-2.5 py-1 rounded-md">
            Section {section.num}
          </span>
          {sp.complete ? (
            <span className="flex items-center gap-1 text-[12px] font-semibold text-green-600 ml-auto">
              <CheckIcon /> Complete
            </span>
          ) : (
            <span className="text-[12px] font-semibold text-gray-400 ml-auto">
              {sp.done} / {sp.total}
            </span>
          )}
        </div>
        <h2 className="font-serif text-[22px] text-navy leading-tight mb-1">{section.title}</h2>
        <p className="text-[13.5px] text-gray-400 italic leading-snug">{section.intro}</p>
      </div>
      ...
      <div className="mt-5 pt-5 border-t border-dashed border-gray-200">
        <label className="flex items-center gap-1.5 text-[11px] font-bold tracking-[1.2px] uppercase text-gray-400 mb-2">
        ...
        <textarea
          ...
          rows={2}
          className="w-full bg-cream-warm border border-gray-200 rounded-lg px-3.5 py-2.5 text-[13px] text-gray-700 resize-y focus:outline-none focus:border-navy focus:bg-white focus:ring-2 focus:ring-navy/8 transition-all placeholder:text-gray-300 font-sans"
        />
      </div>
    </div>
  );
...
async function generatePDF(state) {
  ...
  const PW=216, PH=279, ML=16, MR=16, MT=14, CW=216-16-16;
  ...
  y += 15;
  doc.setDrawColor(...GOLD); doc.setLineWidth(0.5); doc.line(ML,y,ML+CW,y);
  y += 5;
  ...
  TEMPLATE.sections.forEach(section => {
    ...
    section.items.forEach(item => {
      ...
      // Label
      doc.setFont('helvetica', checked ? 'normal' : 'normal');
      doc.setFontSize(8); doc.setTextColor(checked ? 148 : 45, checked ? 163 : 55, checked ? 184 : 72);
      const labelLines = doc.splitTextToSize(item.label, CW-7);
      doc.text(labelLines, ML+6, y+2.5);
      y += labelLines.length * 3.5;

      if (item.note) {
        doc.setFont('helvetica','italic'); doc.setFontSize(7); doc.setTextColor(...GRAY);
        const noteLines = doc.splitTextToSize(item.note, CW-7);
        doc.text(noteLines, ML+6, y+1);
    ...
    if (note && note.trim()) {
      checkPage(12);
      doc.setFillColor(248,246,241); doc.rect(ML, y, CW, 1, 'F');
...
export default function DCFChecklist() {
  ...
  const handleProgramInfo = useCallback((id, val) => {
    setState(prev => ({ ...prev, programInfo: { ...prev.programInfo, [id]: val } }));
  }, []);

  const handleNote = useCallback((id, val) => {
    ...
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-cream-warm">
        ...
        <header className="bg-white border-b border-gray-200 px-6 lg:px-10 h-[68px] flex items-center gap-4 flex-shrink-0">
          <button onClick={() => setSidebar(true)} className="lg:hidden p-2 rounded-lg text-gray-400 hover:bg-gray-100">
          ...
          <nav className="flex items-center gap-2 text-[13px] text-gray-400">
            <span>Resources</span><span className="text-gray-300">/</span>
            <span>Templates</span><span className="text-gray-300">/</span>
            <span className="text-navy font-semibold truncate max-w-[260px]">DCF Facility License Application Checklist</span>
          </nav>
        </header>
        ...
        <div className="bg-white border-b border-gray-200 px-6 lg:px-10 py-3.5 flex items-center gap-4 flex-wrap sticky top-0 z-10 shadow-sm">
          ...
          <div className="flex items-center gap-3">
            <span className="text-[12px] text-gray-400 font-medium hidden sm:block">Progress</span>
          ...
          <button onClick={handleDocx}
            ...
            <DocxIcon /><span className="hidden sm:inline">Blank DOCX</span>
          </button>
        ...
        <div className="px-6 lg:px-10 py-9 max-w-[900px] w-full mx-auto pb-20">
          ...
          <div className="mb-8 pb-7 border-b border-gray-100">
            <span className="inline-block text-[11px] font-bold tracking-[2px] uppercase text-gold mb-3.5">{TEMPLATE.eyebrow}</span>
            <h1 className="font-serif text-[34px] text-navy leading-tight mb-3">{TEMPLATE.title}</h1>
            <p className="text-[15px] text-gray-400 leading-relaxed max-w-2xl">{TEMPLATE.intro}</p>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-7 mb-6 shadow-sm">
            <div className="flex items-center gap-3 mb-5">
              <div className="w-8 h-8 rounded-lg bg-navy-soft flex items-center justify-center text-navy"><FileIcon /></div>
              <h2 className="font-serif text-[20px] text-navy">Program Information</h2>
            </div>
            <div className="grid grid-cols-2 gap-x-5 gap-y-4">
              {TEMPLATE.programInfo.map(f => (
                <div key={f.id} className={f.full ? 'col-span-2' : 'col-span-1 sm:col-span-1'}>
                  <label className="block text-[12px] font-semibold text-navy mb-1.5 tracking-[0.2px]">{f.label}</label>
                  <input
                    type={f.type || 'text'}
                    value={state.programInfo[f.id] || ''}
                    onChange={e => handleProgramInfo(f.id, e.target.value)}
                    className="w-full bg-cream-warm border border-gray-200 rounded-lg px-3 py-2.5 text-[13.5px] text-gray-700 font-sans focus:outline-none focus:border-navy focus:bg-white focus:ring-2 focus:ring-navy/8 transition-all"
                  />
                </div>
              ))}
            </div>
          ...
          <div className="bg-gradient-to-r from-cream to-cream-warm border border-gold/30 border-l-4 border-l-gold rounded-xl p-6 mt-4">
            <div className="text-[11px] font-bold tracking-[2px] uppercase text-gold mb-2">{TEMPLATE.callout.eyebrow}</div>
            <p className="text-[14.5px] text-navy-deep leading-relaxed">{TEMPLATE.callout.body}</p>
          </div>
        </div>
      </main>

...
function OverallBadge({ status }) {
  ...
  return (
    <span
      className={`inline-block px-2.5 py-1 rounded-full text-[10.5px] font-bold whitespace-nowrap
...
async function generatePDF(rows) {
  ...
  if (!filled.length) {
    alert("Add at least one child before downloading the PDF.");
    return;
  }
  ...
  doc.setFontSize(10);
  doc.setTextColor(...NAVY);
  doc.text("Child Enrollment Document Checklist", ML + CW, y + 5, {
    align: "right",
  });
  doc.setFont("helvetica", "normal");
  doc.setFontSize(6.5);
  ...
  doc.text(`Generated ${today} · ${filled.length} children`, ML + CW, y + 9.5, {
    align: "right",
  });
  y += 15;
  ...
  const groups = [
    { label: "CHILD", span: 2 },
    { label: "COMPLEROS TEMPLATES", span: 2 },
    { label: "STATE FORMS", span: 2 },
    { label: "SUPPORTING DOCS", span: 3 },
  ...
  filled.forEach((r, idx) => {
    ...
    const vals = [
      r.name,
      r.enrolled ? "✓" : "—",
      ...DOC_KEYS.map((k) => STATE_LABELS[r[k]] || "—"),
      getOverall(r) || "—",
    ];
    let cellX = ML;
    vals.forEach((v, i) => {
  ...
  doc.save(
    `Compleros_Enrollment_Checklist_${new Date().toISOString().slice(0, 10)}.pdf`,
  );
}
...
export default function EnrollmentChecklist() {
  ...
  const handleReset = () => {
    ...
    try {
      localStorage.removeItem("compleros-enrollment-checklist");
    } catch (e) {}
    setReset(false);
  };
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div
        className={`fixed inset-y-0 left-0 z-50 transform transition-transform lg:relative lg:translate-x-0 lg:flex lg:flex-shrink-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-cream-warm">
        ...
        <header className="bg-white border-b border-gray-200 px-6 lg:px-10 h-[68px] flex items-center gap-4 flex-shrink-0">
          ...
          <a
            href="/"
            className="hidden sm:inline-flex items-center gap-1.5 text-gray-400 text-[13px] font-medium px-2.5 py-1.5 -ml-2.5 rounded-lg hover:text-navy hover:bg-cream-warm transition-all"
          >
            <BackIcon /> Back
          </a>
          <nav className="flex items-center gap-2 text-[13px] text-gray-400">
            ...
            <span className="text-navy font-semibold">
              Child Enrollment Document Checklist
            </span>
          </nav>
        </header>
        ...
        <div className="bg-white border-b border-gray-200 px-6 lg:px-10 py-3.5 flex items-center gap-4 flex-wrap sticky top-0 z-10 shadow-sm">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {summary.complete > 0 && (
              <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border border-green-200 bg-green-50 text-green-700">
                <span className="text-sm font-bold">{summary.complete}</span>{" "}
                complete
              </span>
            )}
            {summary.incomplete > 0 && (
              <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border border-red-200 bg-red-50 text-red-700">
                <span className="text-sm font-bold">{summary.incomplete}</span>{" "}
                incomplete
              </span>
            )}
          </div>
          ...
          >
            <XlsxIcon />
            <span className="hidden sm:inline">Blank XLSX</span>
          </button>
          <button
            onClick={() => generatePDF(rows).catch(console.error)}
            disabled={pdfLoading}
        ...
        <div className="px-6 lg:px-10 py-9 max-w-[1400px] w-full mx-auto pb-20">
          ...
          <div className="mb-8 pb-7 border-b border-gray-100">
            ...
            <p className="text-[15px] text-gray-400 leading-relaxed max-w-2xl">
              Track enrollment document status for every child in your program.
              Click each status badge to cycle through: On File → Missing → N/A.
              The overall column reflects the worst-case status per child.
            </p>
            <div className="inline-flex items-center gap-2 mt-5 px-3.5 py-2 bg-cream-deep rounded-lg text-[12.5px] text-gray-500 font-medium">
              ...
              <span>
                ...
                Compleros never sees or stores what you type here.
              </span>
            </div>
          </div>
              ...
              <table className="w-full border-collapse text-[12.5px]">
                <thead>
                  ...
                  <tr>
                    <th
                      colSpan={2}
                      className="bg-cream text-gold text-[9px] font-bold tracking-[1.5px] uppercase py-1.5 text-center border-b border-gray-200"
                    >
                      CHILD
                    </th>
                    <th
                      colSpan={2}
                      className="bg-cream text-gold text-[9px] font-bold tracking-[1.5px] uppercase py-1.5 text-center border-b  border-l border-gray-100"
                    >
                      COMPLEROS TEMPLATES
                    </th>
                    <th
                    ...
                    <th className="bg-cream text-gold text-[9px] font-bold tracking-[1.5px] uppercase py-1.5 text-center border-b  border-l border-gray-100">
                      OVERALL
                    </th>
                  ...
                  <tr className="bg-navy text-white">
                    <th className="py-2.5 px-3 font-semibold text-[11px] text-left min-w-[160px]">
                      Child Name
                    </th>
                    <th className="py-2.5 px-2 font-semibold text-[11px] text-center min-w-[70px]">
                      Enrolled
                    </th>
                    {COLS.map((c) => (
                      <th
                        key={c.key}
                        className="py-2.5 px-2 font-semibold text-[10px] text-center min-w-[80px] leading-tight whitespace-pre-line"
                      >
                        {c.label}
                      </th>
                    ))}
                    <th className="py-2.5 px-2 font-semibold text-[11px] text-center min-w-[90px]">
                      Status
                    </th>
                    <th className="w-8"></th>
                  </tr>
                ...
                <tbody>
                  {rows.map((row, idx) => {
                    ...
                    return (
                      <tr
                        key={row.id}
                        className="hover:bg-navy/[0.015] even:bg-cream/20 group"
                      >
                        <td className="p-1.5">
                          <input
                            type="text"
                        ...
                        <td className="p-1.5 text-center">
                          <input
                            ...
                            onChange={(e) =>
                              updateRow(row.id, "enrolled", e.target.checked)
                            }
          ...
          <div className="flex gap-3 flex-wrap mb-6">
            {[
              ...
              ["missing", "Missing", "bg-red-50 text-red-700 border-red-200"],
              ["na", "N/A", "bg-gray-100 text-gray-400 border-gray-200"],
            ].map(([, label, cls]) => (
              <span
                key={label}
                className={`px-3 py-1 rounded-full text-[11px] font-bold border ${cls}`}
              >
                {label}
              </span>
            ))}
            <span className="text-[12px] text-gray-400 italic self-center">
              — click any badge to cycle through statuses
            </span>
          </div>

          {/* Callout */}
          <div className="bg-gradient-to-r from-cream to-cream-warm border border-gold/30 border-l-4 border-l-gold rounded-xl p-6">
            <div className="text-[11px] font-bold tracking-[2px] uppercase text-gold mb-2">
              Track this in Compleros
            </div>
            <p className="text-[14.5px] text-navy-deep leading-relaxed">
              With a Basic plan, Compleros automatically alerts you when
              enrollment documents are missing or expired — across every child
              in your program. No more manual file checks.
            </p>
          </div>
        </div>
      </main>
